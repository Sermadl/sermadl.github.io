...
제가 구현할 API Gateway는 Eureka Server와 분리되어 있지 않기 때문에 Eureka Server에 Spring Cloud Gateway 라이브러리를 추가해주겠습니다.<br>
...
Spring Cloud Gateway와 GraphQL 라이브러리를 추가해주었습니다.<br>
...
저는 GraphQL 방식으로 프론트엔드와 통신할 예정이기 때문에 _굉장한 고민_ 끝에 **`Header 기반`** 으로 요청을 구분하기로 하였습니다.
